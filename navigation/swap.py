...
class Robot(threading.Thread):
	...
	self.result = True
	self.endCount = 0
	self.points = getpoint(30,[-25,-25],0.05,'/home/denny/catkin_ws/src/ROS-Academy-for-Beginners/slam_sim_demo/maps/Software_Museum.pgm')
        try:
	...
	startFlag = 0
		...
		print(Now-self.LastTime)
                if (( distance<=0.15 or(math.fabs(speed[0]+speed[1])<=1e-3 and Now-self.LastTime>=2) or startFlag == 0 or (math.fabs(speed[0])<=1e-3 and Now-self.LastTime<90 and Now-self.LastTime>=2)) and self.endMappingFlag == 0):
      		    self.LastTime = Now
			...
			self.waypos[0] = (point[0],point[1],0)
		    except:
			break
